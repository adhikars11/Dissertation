...
\vspace{1em}
\textbf{Main contribution:} Two sectors + endogenous capital + education choice → three sources of inefficiency; Take labor market frictions seriously. 
...
\begin{itemize}
    \item \textbf{Frictionless benchmark}
    \begin{itemize}
        \item Workers move to the sector with the higher payoff
        \item Firms invest until the return to capital equals its cost
    \end{itemize}

    \vspace{3mm}

    \item \textbf{With search frictions + bargaining}
    \begin{itemize}
        \item Workers care about \textbf{expected value}: wage + job-finding prospects
        \item Firms choose capital \textbf{before} bargaining
        \item Once capital is sunk, firms do not keep the full return to it
        \item So firms underinvest
    \end{itemize}

    \vspace{3mm}

    \item \textbf{Why manufacturing is hit harder}
    \begin{itemize}
        \item Manufacturing is more capital intensive
        \item So capital underinvestment is stronger there
        \item Too few manufacturing jobs $\rightarrow$ too little incentive to acquire manufacturing skills
    \end{itemize}
\end{itemize}
\end{frame}
...
\begin{frame}{Illustrative evidence that skill supply responds to skill demand}\label{app:oilshock}

Petroleum engineering prevalence rose the 1970s and 2000s energy boom

\vspace{1mm}
\small
Pattern consistent with \textcolor{blue}{Han and Winters (2020), \emph{Economics of Education Review}}.
...
\vspace{1mm}
{\footnotesize Data source for figure: American Community Survey and Haver.}
{\footnotesize The figure is descriptive and meant to show broad boom-bust co-movement across cohorts, not precise year-by-year timing with spot oil prices.}
...
\item $\beta< \eta$ is empirically plausible:\\
{\footnotesize $\eta \approx 0.5$--$0.7$ (Petrongolo \& Pissarides, 2001), \;
$\beta \approx 0.05$--$0.46$ (Hagedorn \& Manovskii, 2008).}
\hyperlink{app:beta_eta}{\beamerbutton{Appendix}}

\vspace{3mm}

\item Then the economy has \textbf{three wedges:}
\begin{itemize}
    \item \textbf{Capital holdup:} firms underinvest in vacancy capital
    \item \textbf{Standard search inefficiency:} vacancy creation / tightness is inefficient
    \item \textbf{New sectoral choice wedge:} workers under-enter manufacturing
\end{itemize}

\vspace{3mm}

\item Sectoral choice in SP vs.\ DE:
\begin{align*}
r(\epsilon_m-\epsilon_s) &= \textcolor{red}{\frac{\eta}{1-\eta}}
[\theta_mP_kk_m(r+s_m)-\theta_sP_kk_s(r+s_s)] \quad (\text{SP}) \\
r(\epsilon_m-\epsilon_s) &= \textcolor{red}{\frac{\beta}{1-\beta}}
[\theta_mP_kk_m(r+s_m)-\theta_sP_kk_s(r+s_s)] \quad (\text{DE})
\end{align*}

\item If $\beta<\eta$, private returns undervalue entry into $m$
$\rightarrow$ too few workers acquire manufacturing skills.
...
\begin{frame}{Calibration: exogenous and calibrated parameters}

\begin{table}[h!]
\centering
\caption{Exogenously Set Parameters}
\label{table_set_parameters_slides}
\begin{adjustbox}{width=0.92\columnwidth,center}
\begin{tabular}{||c c c c||}\hline
\textbf{Parameter} & \textbf{Description} & \textbf{Value} & \textbf{Source} \\\hline\hline
$\nu$ & CES aggregator coefficient & $2$ & Dimaranan et al. (2006) \\
...
$p_m$ & Manufacturing goods price & $1$ & Normalization \\
$p_s^F$ & Foreign service goods price & $1$ & Normalization \\
$L$ & $L_m + L_s$ & $0.94$ & Agriculture labor force share $\approx 0.06$ \\
$\zeta_m$ & Capital share in manufacturing & $0.65$ & CNEP \\
$\zeta_s$ & Capital share in services & $0.30$ & CNEP \\
\hline
\end{tabular}
\end{adjustbox}
\end{table}
...
\label{table_calibrated_parameters_slides}
\begin{adjustbox}{width=0.95\columnwidth,center}
\begin{tabular}{||c c c c||}\hline
\textbf{Parameter} & \textbf{Description} & \textbf{Value} & \textbf{Data Moment} \\\hline\hline
...
$r$ & Discount rate (annual) & $0.06$ & Capital to output ratio \\
...
\begin{tabular}{||c c||}
\hline
\textbf{Moment} & \textbf{$\Delta$ from DE to SP} \\
\hline\hline
Capital to output ratio, $K/Y$ & $+0.4$ \\
Output, $Y_m$ & $+17.9\%$ \\
Output, $Y_s$ & $+7.4\%$ \\
Industrial labor force, $L_m$ & $+1.0$ pp \\
Industrial employment, $n_m/(n+L_a)$ & $+1.1$ pp \\
Industrial unemployment rate, $u_m/L_m$ & $+0.6$ pp \\
Services unemployment rate, $u_s/L_s$ & $+1.0$ pp \\
Real output, $Y$ & $+11.5\%$ \\
\hline
\end{tabular}
\end{adjustbox}
...
\textbf{Key Takeaways}
\begin{itemize}
  \item \textbf{Real output rises by about $11.5\%$ from DE to SP,} driven primarily by $k_i \uparrow$.
  \medskip
  \item \textbf{Higher $L_m$ and higher $Y_m$:} capital deepening raises labor productivity in both sectors, but more in manufacturing because production is more capital intensive.
  \medskip
  \item \textbf{Unemployment rates are higher in SP:} higher $k_i$ makes vacancy creation more costly, lowers tightness, and raises unemployment in both sectors.
\end{itemize}
...
% in preamble if not already included:
% \usepackage[table]{xcolor}

\begin{frame}{When Hosios holds ($\beta=\eta$), capital subsidy implements the planner}

Under $\beta=\eta$, the only inefficiency is the \textbf{capital holdup wedge}.

Full package $(\tau^{K*}, \tau^{T*}) \rightarrow$ implements SP.

Restricted policy $(\tau^{K*})$ without lump-sum tax \rightarrow$ does not fully implement SP, because removing $\tau^{T*}$ induces over-entry.

\begin{table}[H]
\centering
\caption{\textbf{Equilibrium allocations when $\beta=\eta=0.5$}}
\resizebox{\textwidth}{!}{
\begin{tabular}{||c c c c c||}
\hline
\textbf{Moment} & \cellcolor{gray!15}\textbf{SP} & \cellcolor{gray!15}\textbf{DE1} & \textbf{DE2} & \textbf{DE3} \\
 & \cellcolor{gray!15}\textbf{Planner} & \cellcolor{gray!15}\textbf{Full package} & \textbf{Only $\tau^K_i$} & \textbf{Only uniform $\tau^K$} \\
$\tau^K_i$ & \cellcolor{gray!15}{--} & \cellcolor{gray!15}$\tau^K_i=\tau^{K*}_i$ & $\tau^K_i=\tau^{K*}_i$ & $\tau^K_i=\tau^{K*}_m$ \\
$\tau^T_i$ & \cellcolor{gray!15}{--} & \cellcolor{gray!15}$\tau^T_i=\tau^{T*}_i$ & 0 & 0 \\
\hline\hline
Industry labor force, $L_m$ & \cellcolor{gray!15}$22.7\%$ & \cellcolor{gray!15}$22.7\%$ & $23.2\%$ & $22.7\%$ \\
Industry unemployment rate, $u_m/L_m$ & \cellcolor{gray!15}$7.5\%$ & \cellcolor{gray!15}$7.5\%$ & $6.8\%$ & $6.8\%$ \\
Services unemployment rate, $u_s/L_s$ & \cellcolor{gray!15}$7.9\%$ & \cellcolor{gray!15}$7.9\%$ & $6.8\%$ & $6.8\%$ \\
Capital-to-output ratio, $K/Y$ & \cellcolor{gray!15}$3.3$ & \cellcolor{gray!15}$3.3$ & $3.3$ & $3.1$ \\
Tightness, $\theta_m$ & \cellcolor{gray!15}$0.35$ & \cellcolor{gray!15}$0.35$ & $0.44$ & $0.43$ \\
Tightness, $\theta_s$ & \cellcolor{gray!15}$1.77$ & \cellcolor{gray!15}$1.77$ & $2.52$ & $2.49$ \\
Real output, $Y_m$ & \cellcolor{gray!15}$38.0$ & \cellcolor{gray!15}$38.0$ & $38.3$ & $37.4$ \\
Real output, $Y_s$ & \cellcolor{gray!15}$2.7$ & \cellcolor{gray!15}$2.7$ & $2.7$ & $2.64$ \\
Welfare, $W$ & \cellcolor{gray!15}{4.18} & \cellcolor{gray!15}$4.18$ & $4.15$ & $4.09$ \\
Annual fiscal cost (\% GDP) & \cellcolor{gray!15}{--} & \cellcolor{gray!15}$0$ & $2.4$ & $2.1$ \\
\hline
\end{tabular}}
\end{table}
...
\begin{frame}{When Hosios fails ($\beta<\eta$), training subsidy is also needed}

When $\beta<\eta$, there are \textbf{two wedges}:
(i) excessive vacancy creation and
(ii) a \textbf{sectoral choice wedge} because workers capture too little surplus.

Full package $(\tau^{K**}, \tau^{T**}, \tau^{L**}) \rightarrow$ implements SP.

Restricted package $(\tau^{K**}, \tau^{T**})$ without training subsidy \rightarrow$ corrects vacancy creation, but not the sectoral choice wedge.

\begin{table}[H]
\centering
\caption{\textbf{Equilibrium allocations when $\beta=0.3<\eta=0.5$}}
\resizebox{\textwidth}{!}{
\begin{tabular}{||c c c c c||}
\hline
\textbf{Moment} & \cellcolor{gray!15}\textbf{SP} & \textbf{DE-A} & \cellcolor{gray!15}\textbf{DE-B} & \textbf{DE-C} \\
 & \cellcolor{gray!15}\textbf{Planner} & \textbf{DE $(\beta<\eta)$} & \cellcolor{gray!15}\textbf{Full package} & \textbf{No training subsidy} \\
$\tau^K_i$ & \cellcolor{gray!15}{--} & 0 & \cellcolor{gray!15}$\tau^K_i=\tau^{K**}_i$ & $\tau^K_i=\tau^{K**}_i$ \\
$\tau^T_i$ & \cellcolor{gray!15}{--} & 0 & \cellcolor{gray!15}$\tau^T_i=\tau^{T**}_i$ & $\tau^T_i=\tau^{T**}_i$ \\
$\tau^L_m$ & \cellcolor{gray!15}{--} & 0 & \cellcolor{gray!15}$\tau^L_m=\tau^{L**}_m$ & 0 \\
\hline\hline
Industry labor force, $L_m$ & \cellcolor{gray!15}$22.7\%$ & $21.9\%$ & \cellcolor{gray!15}$22.7\%$ & $20.3\%$ \\
Industry unemployment rate, $u_m/L_m$ & \cellcolor{gray!15}$7.5\%$ & $4.6\%$ & \cellcolor{gray!15}$7.5\%$ & $7.1\%$ \\
Services unemployment rate, $u_s/L_s$ & \cellcolor{gray!15}$7.9\%$ & $4.8\%$ & \cellcolor{gray!15}$7.9\%$ & $8.4\%$ \\
Capital-to-output ratio, $K/Y$ & \cellcolor{gray!15}$3.3$ & $3.0$ & \cellcolor{gray!15}$3.3$ & $3.3$ \\
Tightness, $\theta_m$ & \cellcolor{gray!15}$0.35$ & $0.98$ & \cellcolor{gray!15}$0.35$ & $0.40$ \\
Tightness, $\theta_s$ & \cellcolor{gray!15}$1.77$ & $5.76$ & \cellcolor{gray!15}$1.77$ & $1.59$ \\
Real output, $Y_m$ & \cellcolor{gray!15}$38.0$ & $32.7$ & \cellcolor{gray!15}$38.0$ & $33.8$ \\
Real output, $Y_s$ & \cellcolor{gray!15}$2.7$ & $2.6$ & \cellcolor{gray!15}$2.7$ & $2.7$ \\
Real consumption, $C$ & \cellcolor{gray!15}$4.1$ & $3.8$ & \cellcolor{gray!15}$4.1$ & $3.9$ \\
Welfare, $W$ & \cellcolor{gray!15}$4.2$ & $3.8$ & \cellcolor{gray!15}$4.2$ & $4.0$ \\
\hline
\end{tabular}}
\end{table}
